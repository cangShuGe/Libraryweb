<template>
    <div id="login" class="login_bgColor">
        <div class="login-wrapper">
            <!-- logo -->
            <a class="login_logo" href="localhost:8020" target="_blank">藏书阁</a>
            <!-- wrap -->
...
export default {
    ...
    methods: {
        ...
        verify() {
            ...
            } else if(this.form.username.length < 4 || this.form.username.length > 15){
                this.$message.error('对不起，您的用户名长度不正确')
            } else if (this.form.password.length < 6 || this.form.password.length > 25){
                this.$message.error("对不起，你的密码长度不正确！")
            }else {
                this.login()
            }
        },
